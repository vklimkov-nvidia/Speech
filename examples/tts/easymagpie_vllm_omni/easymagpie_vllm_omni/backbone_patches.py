...
def patch_flash_attention_torch_fallback() -> bool:
    """Use PyTorch SDPA for EasyMagpie backbone FlashAttention calls.

    The converted EasyMagpie vLLM path currently reaches FlashAttention with
    finite QKV tensors at the first Nemotron-H attention layer, then receives
    NaN attention outputs. Keep vLLM's scheduler and KV cache contract intact,
    but replace only that kernel call for EasyMagpie backbone layers.
    """

    patched = 0
    try:
        from vllm.attention.backends.flash_attn import FlashAttentionImpl as LegacyFlashAttentionImpl

        patched += int(_patch_flash_attention_impl(LegacyFlashAttentionImpl, is_v1=False))
    except Exception:
        pass
    try:
        from vllm.v1.attention.backends.flash_attn import FlashAttentionImpl as V1FlashAttentionImpl

        patched += int(_patch_flash_attention_impl(V1FlashAttentionImpl, is_v1=True))
    except Exception:
        pass
    patched += int(_patch_v1_flash_attn_varlen_func())
    if patched:
        logger.info("EasyMagpie FlashAttention torch-SDPA fallback installed on %d implementation(s)", patched)
    return bool(patched)


def patch_final_rmsnorm_native(backbone) -> bool:
    """Run the final Nemotron-H RMSNorm through the fp32 native implementation.

    EasyMagpie feeds the backbone through a persistent ``inputs_embeds`` scratch
    buffer. vLLM's fused CUDA add+RMSNorm path is allowed to mutate its inputs
    in-place; on this checkpoint it leaves the final hidden state non-finite
    even after decoder-layer attention outputs are finite. The native RMSNorm
    path performs the residual add and variance in fp32 and returns a fresh
    tensor, matching the safer reference math closely enough for inference.
    """

    enabled = os.environ.get("EASYMAGPIE_FINAL_RMSNORM_NATIVE", "1").lower()
    if enabled in {"0", "false", "no", "off"}:
        return False

    norm_f = getattr(backbone, "norm_f", None)
    if norm_f is None or not hasattr(norm_f, "forward_native"):
        return False
    if getattr(norm_f, "_easymagpie_native_forward_installed", False):
        return False

    def forward(x, residual=None):
        return norm_f.forward_native(x, residual)

    norm_f.forward = forward
    norm_f._easymagpie_native_forward_installed = True
    logger.info("EasyMagpie final RMSNorm native fp32 fallback installed")
    return True


def patch_layer_rmsnorm_native(backbone) -> int:
    """Run Nemotron-H decoder-layer RMSNorms through native fp32 math.

    The layer RMSNorm modules update the long-lived residual stream consumed by
    every following hybrid layer. If the fused add+RMSNorm kernel writes a
    non-finite residual, the final hidden state can be corrupted even when each
    mixer's direct output looks finite. The native path preserves vLLM's return
    contract while doing the residual add and variance in fp32.
    """

    enabled = os.environ.get("EASYMAGPIE_LAYER_RMSNORM_NATIVE", "1").lower()
    if enabled in {"0", "false", "no", "off"}:
        return 0

    patched = 0
    for layer_idx, layer in enumerate(getattr(backbone, "layers", []) or []):
        norm = getattr(layer, "norm", None)
        if norm is None or not hasattr(norm, "forward_native"):
            continue
        if getattr(norm, "_easymagpie_native_forward_installed", False):
            continue

        def forward(x, residual=None, *, _norm=norm):
            return _norm.forward_native(x, residual)

        norm.forward = forward
        norm._easymagpie_native_forward_installed = True
        norm._easymagpie_native_forward_layer_idx = layer_idx
        patched += 1

    if patched:
        logger.info("EasyMagpie layer RMSNorm native fallback installed on %d module(s)", patched)
    return patched
