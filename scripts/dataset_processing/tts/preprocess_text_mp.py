...
def get_args():
    ...
    parser.add_argument(
        "--joblib_batch_size", default=1, type=int, help="Batch size for joblib workers."
    )
    parser.add_argument(
...
def _process_row(row) -> dict:
    entry, normalizer, text_key, normalized_text_key, lower_case, lower_case_norm = row
    text = entry[text_key]
...
def main():
    ...
    rows = [(entry, normalizer, text_key, normalized_text_key, lower_case, lower_case_norm) for entry in entries]
    with open(output_manifest_path, "w", encoding="utf-8") as output_f:
        with Pool(num_workers) as pool:
            for output_row in tqdm(
                pool.imap(_process_row, rows, chunksize=batch_size), total=len(entries), miniters=1000, maxinterval=600
            ):
                output_line = f"{json.dumps(output_row, ensure_ascii=False)}\n"
