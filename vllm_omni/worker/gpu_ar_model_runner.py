...
try:
    ...
    def apply_grammar_bitmask(*args, **kwargs):
        return None
try:
...
try:
    ...
    def is_residual_scattered_for_sp(*args, **kwargs) -> bool:
        return False
...
def _set_num_tokens_if_present(input_batch: Any, req_idx: int, value: int) -> None:
    num_tokens = getattr(input_batch, "num_tokens", None)
    if num_tokens is not None:
        num_tokens[req_idx] = value
...
class GPUARModelRunner(OmniGPUModelRunner):
    ...
    def _bookkeeping_sync(
        ...
        num_nans_in_logits = {}
        if getattr(envs, "VLLM_COMPUTE_NANS_IN_LOGITS", False) and hasattr(self, "_get_nans_in_logits"):
        ...
        for req_idx, sampled_ids in enumerate(valid_sampled_token_ids):
            ...
            _set_num_tokens_if_present(self.input_batch, req_idx, end_idx)
    ...
    def execute_model(
        ...
        if preempted_req_ids and has_kv_transfer_group():
            get_kv_transfer_group().handle_preemptions(preempted_req_ids)
        ...
        with (
            ...
            has_separate_kv_update = not all(
                all(
                    getattr(g.backend, "forward_includes_kv_cache_update", True)
        ...
        return self.sample_tokens(grammar_output=None)
    ...
    def sample_tokens(
        ...
        with record_function_or_nullcontext("gpu_model_runner: sample"):
            ...
            elif hasattr(self, "_sample"):
                sampler_output = self._sample(logits, spec_decode_metadata)
            else:
                raise NotImplementedError("Speculative sampling is not supported by this compatibility path")
