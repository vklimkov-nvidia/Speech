...
class OmniModelRunnerOutput(ModelRunnerOutput):
    ...
    def __init__(
        ...
        *args: Any,
        multimodal_outputs: dict[str, torch.Tensor] | None = None,
        kv_extracted_req_ids: list[str] | None = None,
        ec_connector_output: Any | None = None,
        cudagraph_stats: Any | None = None,
        **kwargs: Any,
    ) -> None:
        """Bridge Omni-only fields across vLLM ModelRunnerOutput versions."""

        base_params = inspect.signature(ModelRunnerOutput).parameters
        base_kwargs = {key: value for key, value in kwargs.items() if key in base_params}
        if "spec_token_ids" in base_params and "spec_token_ids" not in base_kwargs:
            base_kwargs["spec_token_ids"] = None
        super().__init__(*args, **base_kwargs)
        self.multimodal_outputs = multimodal_outputs
        self.kv_extracted_req_ids = kv_extracted_req_ids
        self.ec_connector_output = ec_connector_output
        self.cudagraph_stats = cudagraph_stats
