...
class OmniRequestState(RequestState):
    ...
    def make_request_output(
        ...
        if self.parent_req is None:
            ...
            parent_result = self.parent_req.get_outputs(self.request_id, output)
            if len(parent_result) == 3:
                parent_request_id, outputs, finished = parent_result
            else:
                outputs, finished = parent_result
                parent_request_id = self.parent_req.request_id
            if not outputs:
                return None
            external_req_id = getattr(self.parent_req, "external_req_id", parent_request_id)
