...
def _upgrade_to_omni_request(
    ...
    fields = set(getattr(OmniEngineCoreRequest, "__struct_fields__", ()))
    kwargs = {
        "request_id": request.request_id,
        "prompt_token_ids": request.prompt_token_ids,
        "mm_kwargs": getattr(request, "mm_kwargs", None),
        "mm_hashes": getattr(request, "mm_hashes", None),
        "mm_placeholders": getattr(request, "mm_placeholders", getattr(request, "mm_positions", None)),
        "mm_features": getattr(request, "mm_features", getattr(request, "mm_kwargs", None)),
        ...
        "current_wave": getattr(request, "current_wave", 0),
        "priority": getattr(request, "priority", 0),
        "trace_headers": getattr(request, "trace_headers", None),
        "resumable": getattr(request, "resumable", False),
        "external_req_id": getattr(request, "external_req_id", None),
        "reasoning_ended": getattr(request, "reasoning_ended", False),
