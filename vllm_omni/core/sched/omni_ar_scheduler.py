...
def _engine_core_output(**kwargs):
    fields = set(getattr(EngineCoreOutput, "__struct_fields__", ()))
    if fields:
        kwargs = {key: value for key, value in kwargs.items() if key in fields}
    return EngineCoreOutput(**kwargs)
...
class OmniARScheduler(VLLMScheduler):
    ...
    def _handle_stopped_request(self, request: Request) -> bool:
        """Compatibility with vLLM builds that removed this scheduler hook."""
        base_handler = getattr(super(), "_handle_stopped_request", None)
    ...
    def schedule(self) -> SchedulerOutput:  # type: ignore[override]
        ...
        try:
            ...
            for nr in scheduler_output.scheduled_new_reqs:
                ...
                # Build omni entry preserving all base fields. vLLM changed
                # NewRequestData multimodal field names across releases, so
                # filter constructor kwargs at runtime and attach optional
                # attrs afterward for downstream Omni code.
                mm_kwargs = list(getattr(nr, "mm_kwargs", None) or getattr(nr, "mm_features", None) or [])
                mm_hashes = list(getattr(nr, "mm_hashes", None) or [])
                mm_positions = list(getattr(nr, "mm_positions", None) or [])
                payload = dict(
                    req_id=nr.req_id,
                ...
                fields = getattr(OmniNewRequestData, "__dataclass_fields__", {})
                omni_nr = OmniNewRequestData(**{name: value for name, value in payload.items() if name in fields})
                for name, value in (
                    ("mm_kwargs", mm_kwargs),
                    ("mm_hashes", mm_hashes),
                    ("mm_positions", mm_positions),
                ):
                    if not hasattr(omni_nr, name):
                        setattr(omni_nr, name, value)
    ...
    def update_from_output(
        ...
        prompt_logprobs_dict = getattr(model_runner_output, "prompt_logprobs_dict", {})
        num_scheduled_tokens = scheduler_output.num_scheduled_tokens
        pooler_outputs = getattr(model_runner_output, "pooler_output", [])
        num_nans_in_logits = getattr(model_runner_output, "num_nans_in_logits", {})
        kv_connector_output = getattr(model_runner_output, "kv_connector_output", None)
        cudagraph_stats: CUDAGraphStat | None = getattr(model_runner_output, "cudagraph_stats", None)
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            if stopped:
                routed_experts = self._get_routed_experts(request) if hasattr(self, "_get_routed_experts") else None
            ...
            if new_token_ids or pooler_output is not None or kv_transfer_params or stopped:
                ...
                outputs[request.client_index].append(
                    _engine_core_output(
                        ...
                        num_nans_in_logits=getattr(request, "num_nans_in_logits", 0),
                    )
                )
                if self.chunk_transfer_adapter is not None:
